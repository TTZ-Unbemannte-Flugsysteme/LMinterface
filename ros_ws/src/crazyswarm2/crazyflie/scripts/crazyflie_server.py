...
class CrazyflieServer(Node):
    def __init__(self):
        ...
        for link_uri in self.uris:
            ...
            # link statistics from CFlib (may not be available in older cflib versions like CrazySim)
            ...
            try:
                if hasattr(self.swarm._cfs[link_uri].cf, 'link_statistics'):
                ...
                self.get_logger().warning(f"[{self.cf_dict[link_uri]}] link_statistics not available (older cflib version)")
    ...
    def _poses_changed(self, msg):
        ...
        Topic update callback to the motion capture lib's
           ...
           to the crazyflie
        """
        # Rate limiting: only send external pose at max 50Hz to reduce radio bandwidth
        current_time = self.get_clock().now().nanoseconds
        if not hasattr(self, '_last_extpos_time'):
            self._last_extpos_time = {}
        ...
        for pose in poses:
            ...
            if name in self.uri_dict.keys():
                ...
                # Check if enough time has passed (20ms = 50Hz)
                last_time = self._last_extpos_time.get(uri, 0)
                if (current_time - last_time) < 20_000_000:  # 20ms in nanoseconds
                    continue  # Skip this update, too soon
                self._last_extpos_time[uri] = current_time
